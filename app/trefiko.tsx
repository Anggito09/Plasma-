...
type Order = { id: string; day: string; number: number; customer: string; mode: string; items: Item[]; note: string; total: number; payment: string; status: string; created_at: number; version: number; cancel_reason?: string };
...
export default function Trefiko({ view = 'kasir' }: { view?: string }) {
  ...
  return (
    <>
      <div className="app-shell no-print">
        ...
        {actualView === 'kasir' && <Cashier products={products} board={board} config={config} connected={connected} onSave={async (b) => {
          let d;
        ...
        {actualView === 'pengaturan' && isAdmin && <SettingsPanel config={config} products={products} onSave={async (path, b) => {
      ...
      <section className="auth-form">
        {boot === 'loading' ? <><Loader2 className="spin" /><p>Menghubungkan…</p></> : (
          <form onSubmit={async (e) => {
            ...
            <p className="login-help"><ShieldCheck size={15} /> Sesi 12 jam</p>
          </form>
        )}
      </section>
    </main>
...
function Cashier({ products, board, config, connected, onSave, onReceipt }: { products: Product[]; board: Board | null; config: Config; connected: boolean; onSave: (b: any) => Promise<any>; onReceipt: (o: Order) => void }) {
      ...
      <div className="cashier-grid">
        <section className="menu-area">
          ...
          <div className="menu-grid">
            {filtered.map(p => {
              ...
              return (
                <button disabled={busy} className={'menu-card' + (qty ? ' in-cart' : '')} key={p.id} onClick={() => add(p)}>
                  <span className={'menu-symbol category-' + (p.category === 'Kopi' ? 'coffee' : p.category === 'Makanan' ? 'food' : 'drink')}>{p.category === 'Makanan' ? <Utensils size={28} /> : <Coffee size={28} />}</span>
        ...
        <aside className="cart">
          ...
          <form onSubmit={async (e) => {
            ...
            setBusy(true);
            const data = { customer: customer.trim(), mode, payment, note, expectedTotal: total, items: cart.map(i => ({ id: i.id, quantity: i.quantity })) };
            ...
            <fieldset disabled={busy}>
              ...
              <label>Bayar pakai<SelectField label="Pembayaran" value={payment} onChange={setPayment} options={[['cash', 'Tunai'], ['qris', 'QRIS'], ['card', 'Kartu']]} /></label>
  ...
  return (
    <section>
      <div className="section-top kitchen-toolbar">
        ...
        <span className="muted">Otomatis tiap 3 dtk</span>
      </div>
    ...
    <main className="display">
      ...
      <div className="display-main">
        ...
        <section className="preparing-display">
          ...
          <div className="display-queue">{waiting.slice((rotation % pages) * 12, (rotation % pages) * 12 + 12).map(o => <div key={o.id}><strong>{queueNumber(o.number)}</strong><span>{o.customer}</span><small>{statusNames[o.status]}</small></div>)}</div>
  ...
  return (
    <section>
      ...
      <div className="table-panel">
        <Table><TableHeader><TableRow>{['Antrean', 'Pelanggan', 'Jam', 'Jenis', 'Total', 'Status', ''].map(t => <TableHead key={t}>{t}</TableHead>)}</TableRow></TableHeader>
...
function SettingsPanel({ config, products, onSave, onLogout }: { config: Config; products: Product[]; onSave: (path: string, b: any) => Promise<any>; onLogout: () => void }) {
