...
async function handle(request: Request) {
  ...
  const standalone=(env as unknown as Record<string,string>).PLASMA_STANDALONE==='true';
  const trustedSetup=!standalone&&!!request.headers.get('oai-authenticated-user-id');
  return createService(env.DB,{setupKey:(env as unknown as Record<string,string>).PLASMA_SETUP_KEY||'',trustedSetup})(request);
}
