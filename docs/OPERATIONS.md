...
1. Open Plasma on the cashier device and sign in with a cashier account.
...
7. Cashier/admin can cancel an active order with a reason. Refunds are processed outside Plasma.
...
If a save fails after an uncertain connection result, retry the same unchanged order. Plasma reuses its request ID, so the server returns the previously saved order instead of allocating another number. Do not change the draft or reload until checking history for the original order. Browser drafts are not durable, and offline order entry is not supported.
