...
function database() {
    ...
    sqlite.exec(readFileSync(new URL('../drizzle/0002_typical_dazzler.sql', import.meta.url), 'utf8'));
    sqlite.exec(readFileSync(new URL('../drizzle/0003_organic_blue_marvel.sql', import.meta.url), 'utf8'));
...
async function fixture() {
    ...
    let cookie = '';
    const call = async (path, body, custom = {}) => { const headers = { origin: 'https://temancipta.test', ...(body ? { 'content-type': 'application/json' } : {}), cookie, ...custom }; const r = await service(new Request('https://temancipta.test/api/' + path, { method: body ? 'POST' : 'GET', headers, body: body ? JSON.stringify(body) : undefined })); return { status: r.status, data: await r.json(), cookie: r.headers.get('set-cookie')?.split(';')[0] }; };
    ...
    const order = (overrides = {}) => ({ id: crypto.randomUUID(), customer: 'Anggito', mode: 'dine-in', payment: 'Tunai', note: 'Tanpa gula', items: [{ id: products[0].id, quantity: 2 }], ...overrides });
...
test('admin kelola metode bayar; kasir pakai BRI; metode asing ditolak', async () => { const f = await fixture(); const seed = (await f.call('payments')).data.payments.map(p => p.name); assert.ok(seed.includes('Tunai') && seed.includes('QRIS') && seed.includes('Kartu')); assert.equal((await f.call('payments', { name: 'BRI' })).status, 200); assert.equal((await f.call('payments', { name: 'Mandiri' })).status, 200); const list = (await f.call('payments')).data.payments; assert.ok(list.some(p => p.name === 'BRI')); const o = await f.call('orders', f.order({ payment: 'BRI' })); assert.equal(o.status, 201); assert.equal(o.data.order.payment, 'BRI'); assert.equal((await f.call('orders', f.order({ payment: 'BCA' }))).status, 400); f.sqlite.close(); });
test('waktu tahap tercatat; riwayat tampilkan rata-rata siap & ambil', async () => { const f = await fixture(); const t0 = f.getClock(); const o = (await f.call('orders', f.order())).data.order; f.setClock(t0 + 10000); await f.call('orders/action', { id: o.id, action: 'prepare', version: 1, requestId: crypto.randomUUID() }); f.setClock(t0 + 20000); await f.call('orders/action', { id: o.id, action: 'ready', version: 2, requestId: crypto.randomUUID() }); f.setClock(t0 + 35000); await f.call('orders/action', { id: o.id, action: 'complete', version: 3, requestId: crypto.randomUUID() }); const h = (await f.call('history?day=2026-09-11')).data; assert.equal(h.summary.avgReady, 20000); assert.equal(h.summary.avgTake, 15000); assert.equal(h.summary.nReady, 1); assert.equal(h.orders[0].ready_at - h.orders[0].created_at, 20000); f.sqlite.close(); });
